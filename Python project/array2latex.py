...
def tolatex(array, header, caption="My table", label="table_something", output_file=False):
    ...
    padding = " " * (max(length_table_items, length_header_items) // 2 - 2) + "&" + " " * (
            max(length_table_items, length_header_items) // 2 - 2)
    ...
    for regex_repl in zip((regex_space_bracket, regex_1, regex_2, regex_3),
